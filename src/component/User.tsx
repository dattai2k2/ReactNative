...
const User = ({ item }:any) => {
  ...
  //         `http://10.0.1.3:8000/friend-requests/sent/${userId}`
  //       );
  ...
  //     try {
  //       const response = await fetch(`http://10.0.1.3:8000/friends/${userId}`);
  ...
  const sendFriendRequest = async (currentUserId: any, selectedUserId: any) => {
    try {
      const response = await fetch(api.port+"/friend-request", {
        method: "POST",
        headers: {
